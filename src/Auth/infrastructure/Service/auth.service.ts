...
import { IAuthService, TUserOptionsLogin, TUserOptionsSignup } from "./auth.service.types";
...
class _AuthService extends AuthJWTService implements IAuthService {
    ...
    async login(userOptions: TUserOptionsLogin): Promise<ITokenResponse> {
        try {
            const potentialUser = await this.userService.findByEmail(userOptions.email);
            ...
            const userToken = this.createToken({ email: potentialUser.email, id: potentialUser.id });
    ...
    async signup(userOptions: TUserOptionsSignup): Promise<{ data: User; error: null }> {
        ...
        const newUser = await this.userService.create({ email, name, password: hashedPassword });
